...
console.log(name)
  ...
  const mapStyles = {        
    ...
    const onSelect = item => {
      console.log(item)
      setSelected(item);
      axios.get(`http://localhost:8080/api/driver/${item.driver}`,   { headers } )
      .then(res=>{
    ...
    { coords.map(item => {
         ...
         {console.log(selected.location)}
         {
            ...
            (
              ...
              clickable={true}
              onCloseClick={()=> setWork()}
            >
               <div > 
               <Avatar    src ={`https://malta-images.s3.amazonaws.com/${pic}`} size="70"  round={true}   />
               <p style= {{ color: "blue"}}>Name: {name}</p>
               
               <p style= {{ color: "green"}}> City: {selected.city}</p>
               </div>
